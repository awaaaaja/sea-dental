...
async function handleDelete() {
  ...
  deleting.value = true
  await supabase.from('services').delete().eq('id', deleteId.value)
  deleteId.value = null
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="font-display text-[20px] md:text-[24px] font-bold text-gray-900">Layanan</h1>
        <p class="font-body text-sm text-gray-500 mt-1">Kelola semua layanan klinik</p>
      </div>
      <router-link to="/admin/services/new"
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-gray-100 bg-gray-50/50">
              ...
              <th class="text-left px-5 py-3 text-[11px] font-display font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Icon</th>
              <th class="text-center px-5 py-3 text-[11px] font-display font-semibold text-gray-500 uppercase tracking-wider">Status</th>
              <th class="text-center px-5 py-3 text-[11px] font-display font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Urutan</th>
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-if="loading" v-for="i in 5" :key="i" class="animate-pulse">
              ...
              <td class="px-5 py-4 hidden lg:table-cell"><div class="h-4 bg-gray-100 rounded w-16"></div></td>
              <td class="px-5 py-4"><div class="h-5 bg-gray-100 rounded-full w-16 mx-auto"></div></td>
              <td class="px-5 py-4 hidden md:table-cell"><div class="h-4 bg-gray-100 rounded w-8 mx-auto"></div></td>
              <td class="px-5 py-4"><div class="h-4 bg-gray-100 rounded w-16 ml-auto"></div></td>
            </tr>
            <tr v-else-if="services.length === 0">
              <td colspan="6" class="px-5 py-12 text-center">
                <span class="material-symbols-outlined text-5xl text-gray-200 mb-3 block">medical_services</span>
                <p class="text-sm text-gray-400 font-body mb-4">Belum ada layanan</p>
                <router-link to="/admin/services/new" class="text-sm text-primary font-display font-semibold hover:text-cyan-tech">
                  + Tambah Layanan
                </router-link>
              </td>
            </tr>
            <tr v-for="svc in services" :key="svc.id" class="hover:bg-gray-50/50 transition-colors">
              <td class="px-5 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                    <span class="material-symbols-outlined text-primary text-base">{{ svc.icon }}</span>
                  </div>
                  <div class="min-w-0">
                    <p class="text-sm font-display font-medium text-gray-900 truncate">{{ svc.name }}</p>
                    <p class="text-[11px] text-gray-400 font-body truncate">{{ svc.slug }}</p>
                  </div>
                </div>
              </td>
              <td class="px-5 py-4 hidden md:table-cell">
                <span class="text-xs text-gray-600 font-body">{{ svc.service_categories?.name || '-' }}</span>
              </td>
              <td class="px-5 py-4 hidden lg:table-cell">
                <span class="material-symbols-outlined text-gray-400 text-lg">{{ svc.icon }}</span>
              </td>
              <td class="px-5 py-4 text-center">
                <button @click="toggleStatus(svc.id, svc.status)"
                  :class="[
                    'inline-flex px-2.5 py-1 rounded-full text-[11px] font-display font-semibold transition-colors',
                    svc.status === 'published' ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' :
                    svc.status === 'draft' ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' :
              ...
              <td class="px-5 py-4 text-center hidden md:table-cell">
                <span class="text-sm text-gray-500 font-body">{{ svc.sort_order }}</span>
              </td>
              <td class="px-5 py-4">
                <div class="flex items-center justify-end gap-1">
                  <router-link :to="`/admin/services/${svc.id}/edit`"
                    class="p-1.5 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/10 transition-colors">
                    <span class="material-symbols-outlined text-lg">edit</span>
                  </router-link>
                  <button @click="deleteId = svc.id"
        ...
        <div v-if="deleteId" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" @click.self="deleteId = null">
          <div class="bg-white rounded-2xl p-4 w-full max-w-sm shadow-xl">
            <div class="text-center">
              ...
              <h3 class="font-display text-base font-semibold text-gray-900 mb-2">Hapus Layanan?</h3>
